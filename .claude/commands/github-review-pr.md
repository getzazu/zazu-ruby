...
2. Reproduce locally:
   - Spec failure → `bundle exec rspec path/to/spec.rb`
   - Rubocop failure → `bundle exec rubocop`
   - Release-pipeline failure → check `.github/workflows/release.yml` and verify the trusted-publishing binding on rubygems.org
3. Fix the root cause. Don't:
   ...
   - Change `find` to `find_by` to make a NotFoundError go away
4. Verify locally: `bundle exec rake default`
...
6. **Verify** after each batch of fixes: `bundle exec rake default`
...
2. **Type-safety / nil-safety** — nil paths the code didn't handle, missing rescues at boundaries.
3. **Style / lint** — only fix if rubocop autocorrect doesn't already cover it.
